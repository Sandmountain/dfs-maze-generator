...
export class Maze {
  constructor(rows, columns, height, width) {
    ...
    for (let i = 0; i < rows; i++) {
      for (let j = 0; j < columns; j++) {
        const newCell = new Cell(j, i, height, width, rows, columns);
  ...
  dfs() {
    ...
    let topNeighbour = currElm && this.cells[this.getIndex(currElm.i, currElm.j - 1)];
    let rightNeighbour = currElm && this.cells[this.getIndex(currElm.i + 1, currElm.j)];
    let bottomNeighbour = currElm && this.cells[this.getIndex(currElm.i, currElm.j + 1)];
    let leftNeighbour = currElm && this.cells[this.getIndex(currElm.i - 1, currElm.j)];
    topNeighbour &&
  ...
  // Function to get the index of the index for neighbouring cells in a
  // 1-dim array
  getIndex(i, j) {
